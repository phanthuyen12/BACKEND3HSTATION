...
Server sẽ chạy tại: `https://api.3hstation.com`

## 2. Đăng ký tài khoản đầu tiên

```bash
curl -X POST https://api.3hstation.com/api/auth/register \
...
```bash
curl -X POST https://api.3hstation.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "admin@example.com",
    "password": "password123"
...
```bash
curl -X POST https://api.3hstation.com/api/elearning/categories \
...
```bash
curl -X POST https://api.3hstation.com/api/elearning/courses \
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "https://api.3hstation.com/api/client/elearning/courses?page=1&limit=20"
```

### Tạo gói VPS

```bash
curl -X POST https://api.3hstation.com/api/vps/plans \
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  -H "Content-Type: application/json" \
  -d '{
    "id": "vps-basic",
...
2. Set environment variable `base_url` = `https://api.3hstation.com/api`
